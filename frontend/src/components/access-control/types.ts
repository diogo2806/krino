...
export type AccessEvent = { clientEventId: string; studentId: number; registration: string; studentName: string; classId?: number; className?: string; schoolId: number; schoolName: string; eventType: 'ENTRY' | 'EXIT'; capturedAt: string; receivedAt: string; capturedOffline: boolean; sourceType: 'QR' | 'MANUAL'; synchronizedEvent: boolean; duplicate: boolean; notificationDelivered: boolean; };
