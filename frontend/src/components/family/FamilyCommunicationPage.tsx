...
export function FamilyCommunicationPage({ context, onUnauthorized }: Props) {
  const [schools, setSchools] = useState<FamilySchool[]>([]); const [schoolId, setSchoolId] = useState(''); const [classes, setClasses] = useState<FamilyClass[]>([]); const [students, setStudents] = useState<FamilyStudentOption[]>([]); const [conversations, setConversations] = useState<Conversation[]>([]); const [announcements, setAnnouncements] = useState<Announcement[]>([]); const [search, setSearch] = useState(''); const [tab, setTab] = useState<Tab>('messages'); const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [denied, setDenied] = useState(false); const [conversationOpen, setConversationOpen] = useState(false); const [conversation, setConversation] = useState<Conversation>(); const [announcementOpen, setAnnouncementOpen] = useState(false); const [announcementToDeactivate, setAnnouncementToDeactivate] = useState<Announcement>();
  ...
  const loadSchoolData = useCallback(async () => {
    ...
    try {
      ...
      setClasses(nextClasses); setStudents(nextStudents); setConversations(nextConversations); setAnnouncements(nextAnnouncements); setDenied(false);
  ...
  async function deactivateAnnouncement() {
    ...
    try { await apiRequest(`/family-communication/announcements/${announcementToDeactivate.id}`, { method: 'DELETE' }); setAnnouncementToDeactivate(undefined); await loadSchoolData(); }
    catch (exception) { setAnnouncementToDeactivate(undefined); setError(exception instanceof Error ? exception.message : 'Não foi possível desativar o comunicado.'); }
  }
  ...
  return <main className="app-page"><PageHeader eyebrow="Família" title="Comunicação com Famílias" description="Mensagens rastreáveis e comunicados para responsáveis vinculados." manualSections={manualSections} actions={canWrite ? tab === 'messages' ? <Button type="button" variant="primary" onClick={() => { setConversation(undefined); setConversationOpen(true); }}><MessageSquarePlus aria-hidden="true" size={18} />Nova conversa</Button> : <Button type="button" variant="primary" onClick={() => setAnnouncementOpen(true)}><Megaphone aria-hidden="true" size={18} />Novo comunicado</Button> : undefined} />
    ...
    <ConfirmDialog open={Boolean(announcementToDeactivate)} title="Desativar comunicado" message={`Desativar o comunicado “${announcementToDeactivate?.title ?? ''}”? Ele deixará de aparecer no Portal do Responsável, mas o histórico será preservado.`} confirmLabel="Desativar comunicado" onConfirm={() => void deactivateAnnouncement()} onClose={() => setAnnouncementToDeactivate(undefined)} />
  </main>;
}
