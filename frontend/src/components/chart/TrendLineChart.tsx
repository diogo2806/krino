...
export function TrendLineChart({ title, points, emptyMessage }: Props) {
  ...
  return <figure className="chart-card"><figcaption>{title}</figcaption><svg className="trend-chart" viewBox="0 0 520 190" role="img" aria-label={title}><line x1="50" y1="145" x2="470" y2="145" className="trend-chart__axis" /><line x1="50" y1="40" x2="50" y2="145" className="trend-chart__axis" /><polyline points={polyline} className="trend-chart__line" />{valid.map((point) => <g key={point.label}><circle cx={x(point.index)} cy={y(point.value!)} r="5" className="trend-chart__point" /><text x={x(point.index)} y="168" textAnchor="middle" className="trend-chart__label">{point.label}</text><text x={x(point.index)} y={y(point.value!) - 10} textAnchor="middle" className="trend-chart__value">{point.value!.toFixed(1)}%</text></g>)}</svg></figure>;
}
