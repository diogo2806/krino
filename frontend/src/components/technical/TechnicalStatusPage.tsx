...
import { ScreenManual } from '../manual/ScreenManual';
...
const manualSections = [
  ...
  { title: 'Ações', content: 'A verificação da API e do banco ocorre automaticamente ao carregar a tela. O botão Manual abre estas orientações.' },
...
export function TechnicalStatusPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl.replace(/\/$/, '')}/health`).then(async (response) => {
      ...
      setApiStatus(body.application === 'UP' ? 'OK' : 'ERRO');
      setDatabaseStatus(body.database === 'UP' ? 'OK' : 'ERRO');
    }).catch(() => { setApiStatus('ERRO'); setDatabaseStatus('ERRO'); });
  }, []);
  return (
    <main className="technical-page">
      <header className="page-header"><div><p className="eyebrow">KRINO</p><h1>Validação técnica</h1><p className="page-header__description">Estado mínimo da aplicação para implantação no EasyPanel.</p></div><ScreenManual screenName="Validação técnica" sections={manualSections} /></header>
      <section className="status-grid" aria-label="Estado dos serviços"><StatusCard label="Frontend" status="OK" /><StatusCard label="API" status={apiStatus} /><StatusCard label="Banco conectado" status={databaseStatus} /></section>
    </main>
  );
}
