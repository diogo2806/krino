...
import type { BreakdownItem, IndicatorRecord, MonitoringClass, MonitoringContext, MonitoringStudent, MonitoringSummary, SourceMetric, TrendPoint } from './types';

type Props = { context: AccessContext; onUnauthorized: () => void; };

const manualSections = [
  { title: 'Finalidade', content: 'Acompanhar resultados pedagógicos consolidados da Rede, unidade escolar, turma e estudante com base nos dados persistidos nos módulos do KRINO.' },
  { title: 'Campos e filtros', content: 'Ano letivo, período, unidade escolar, turma, estudante e fonte de resultados definem a análise. Quando a conta possui escopo municipal, deixar Unidade escolar em Rede municipal apresenta a visão da Rede.' },
  ...
  { title: 'Botões e ações', content: 'Registrar indicador guarda resultado observado com fonte documentada ou simulação/projeção não oficial no nível atualmente selecionado: Rede, Escola, Turma ou Estudante. O sistema não calcula IDEB/IDEPE oficial sem fórmula confirmada.' },
  { title: 'Regras', content: 'Os filtros afetam todos os cards e gráficos. O estudante só pode ser selecionado dentro de uma matrícula real da turma e ano. Resultados de fontes diferentes permanecem identificados. Simulações e projeções são sempre exibidas como não oficiais e preservam origem e premissas.' },
  { title: 'Permissões', content: 'MONITORING_READ permite consulta no escopo atribuído. MONITORING_MANAGE permite registrar referências, simulações e projeções. A Rede completa exige permissão municipal; escola, turma e estudante respeitam a unidade autorizada.' },
  { title: 'Fluxos', content: 'Selecione o nível, período e fonte, analise os cards, acompanhe a evolução, compare escolas, turmas ou estudantes e consulte referências IDEB/IDEPE. Novas fontes integradas passam a ficar disponíveis no mesmo filtro.' },
...
export function MonitoringPage({ context, onUnauthorized }: Props) {
  ...
  const [year, setYear] = useState(currentYear.toString()); const [period, setPeriod] = useState(''); const [schoolId, setSchoolId] = useState(''); const [classId, setClassId] = useState(''); const [studentId, setStudentId] = useState(''); const [sourceCode, setSourceCode] = useState('INTERNAL_DIARY');
  const [monitoringContext, setMonitoringContext] = useState<MonitoringContext>(); const [classes, setClasses] = useState<MonitoringClass[]>([]); const [students, setStudents] = useState<MonitoringStudent[]>([]); const [summary, setSummary] = useState<MonitoringSummary>(); const [trend, setTrend] = useState<TrendPoint[]>([]); const [breakdown, setBreakdown] = useState<BreakdownItem[]>([]); const [records, setRecords] = useState<IndicatorRecord[]>([]);
  ...
  const selectedSchool = monitoringContext?.schools.find((school) => school.id.toString() === schoolId); const selectedClass = classes.find((item) => item.id.toString() === classId); const selectedStudent = students.find((item) => item.id.toString() === studentId);
  ...
  const loadClasses = useCallback(async () => {
    if (!schoolId) { setClasses([]); setClassId(''); setStudents([]); setStudentId(''); return; }
  ...
  const loadStudents = useCallback(async () => {
    ...
    try { const next = await apiRequest<MonitoringStudent[]>(`/pedagogical-monitoring/students?classId=${classId}&year=${year}`); setStudents(next); setStudentId((current) => current && next.some((item) => item.id.toString() === current) ? current : ''); }
    catch (exception) { setError(exception instanceof Error ? exception.message : 'Não foi possível carregar os estudantes da turma.'); }
  }, [classId, year]);

  const loadData = useCallback(async () => {
    if (!monitoringContext || (!monitoringContext.networkView && !schoolId)) return;
    setLoading(true); setError(''); setDenied(false);
    const scope = `${schoolId ? `&schoolId=${schoolId}` : ''}${classId ? `&classId=${classId}` : ''}${studentId ? `&studentId=${studentId}` : ''}`; const periodParam = period ? `&period=${period}` : '';
    try {
      const [nextSummary, nextTrend, nextBreakdown, nextRecords] = await Promise.all([
        ...
        apiRequest<BreakdownItem[]>(`/pedagogical-monitoring/breakdown?year=${year}${periodParam}${schoolId ? `&schoolId=${schoolId}` : ''}${classId ? `&classId=${classId}` : ''}`),
        apiRequest<IndicatorRecord[]>(`/pedagogical-monitoring/indicator-records?year=${year}${scope}`),
      ]);
  ...
  }, [monitoringContext, year, period, schoolId, classId, studentId, onUnauthorized]);

  useEffect(() => { void loadContext(); }, [loadContext]); useEffect(() => { void loadClasses(); }, [loadClasses]); useEffect(() => { void loadStudents(); }, [loadStudents]); useEffect(() => { void loadData(); }, [loadData]);

  const selectedMetric = summary ? metricForSource(summary.sources, sourceCode) : undefined;
  const levelLabel = studentId ? selectedStudent?.name ?? 'Estudante' : classId ? selectedClass?.name ?? 'Turma' : schoolId ? selectedSchool?.name ?? 'Unidade escolar' : 'Rede municipal';
  ...
  const breakdownTitle = classId ? 'Comparação entre estudantes' : schoolId ? 'Comparação entre turmas' : 'Comparação entre unidades escolares';
