...
import type { AccessNotification, Announcement, AttendanceSummary, Conversation, LinkedStudent, ReportCard } from './types';
...
export function FamilyPortalPage({ context, onUnauthorized }: Props) {
  ...
  const [report, setReport] = useState<ReportCard>(); const [attendance, setAttendance] = useState<AttendanceSummary>(); const [notifications, setNotifications] = useState<AccessNotification[]>([]); const [announcements, setAnnouncements] = useState<Announcement[]>([]); const [conversations, setConversations] = useState<Conversation[]>([]);
  ...
  const yearOptions = useMemo(() => Array.from(new Set([currentYear - 2, currentYear - 1, currentYear, currentYear + 1, ...students.map((item) => item.academicYear).filter((value): value is number => value != null)])).sort((a, b) => b - a).map((value) => ({ value: value.toString(), label: value.toString() })), [currentYear, students]);

  const loadStudents = useCallback(async () => {
    setLoading(true); setError('');
    try {
      const next = await apiRequest<LinkedStudent[]>('/family-portal/students');
      setStudents(next);
      setStudentId((current) => current && next.some((item) => item.id.toString() === current) ? current : next[0]?.id.toString() ?? '');
      if (next[0]?.academicYear) setYear(next[0].academicYear.toString());
      setDenied(false);
    } catch (exception) {
  ...
  const loadStudentData = useCallback(async () => {
    ...
    setLoading(true); setError('');
    try {
      const [nextReport, nextAttendance, nextNotifications, nextAnnouncements, nextConversations] = await Promise.all([
        apiRequest<ReportCard>(`/family-portal/students/${studentId}/report-card?year=${year}&period=${period}`),
        apiRequest<AttendanceSummary>(`/family-portal/students/${studentId}/attendance?year=${year}&period=${period}`),
      ...
      setReport(nextReport); setAttendance(nextAttendance); setNotifications(nextNotifications); setAnnouncements(nextAnnouncements); setConversations(nextConversations); setDenied(false);
    } catch (exception) {
      if (exception instanceof ApiError && exception.status === 401) { onUnauthorized(); return; }
      if (exception instanceof ApiError && exception.status === 403) { setDenied(true); return; }
  ...
  return <main className="app-page"><PageHeader eyebrow="Família" title="Portal do Responsável" description={selectedStudent ? `Estudante selecionado: ${selectedStudent.name}.` : `Olá, ${context.displayName}. Acompanhe os estudantes vinculados à sua conta.`} manualSections={manualSections} />
    ...
    {students.length > 0 ? <FilterBar><SelectField name="familyStudent" label="Estudante" value={studentId} onChange={(event) => { const nextId = event.target.value; setStudentId(nextId); const nextStudent = students.find((item) => item.id.toString() === nextId); if (nextStudent?.academicYear) setYear(nextStudent.academicYear.toString()); }} options={students.map((item) => ({ value: item.id.toString(), label: `${item.name}${item.className ? ` · ${item.className}` : ''}` }))} /><SelectField name="familyYear" label="Ano letivo" value={year} onChange={(event) => setYear(event.target.value)} options={yearOptions} /><SelectField name="familyPeriod" label="Período" value={period} onChange={(event) => setPeriod(event.target.value)} options={[1,2,3,4].map((value) => ({ value: value.toString(), label: `${value}º período` }))} /></FilterBar> : null}
    {students.length === 0 && !loading ? <StateMessage title="Nenhum estudante vinculado" message="Sua conta ainda não possui estudante autorizado. Solicite à Administração responsável pelo acesso a validação do vínculo legal." /> : null}
    ...
    {!loading && studentId && tab === 'report' ? <section className="family-panel"><h2>Boletim</h2>{report && (report.components.length > 0 || report.assessments.length > 0) ? <><div className="family-result-grid">{report.components.map((item) => <article className="family-result-card" key={item.componentId}><strong>{item.componentName}</strong><span>Nota consolidada: {item.grade == null ? 'Ainda não informada' : item.grade.toLocaleString('pt-BR')}</span><span>Faltas consolidadas: {item.absences}</span></article>)}</div>{report.assessments.length ? <div className="family-assessment-list">{report.assessments.map((item, index) => <article key={`${item.title}-${index}`}><strong>{item.componentName} · {item.title}</strong><span>{item.score == null ? 'Sem nota' : `${item.score.toLocaleString('pt-BR')}${item.maxScore != null ? ` de ${item.maxScore.toLocaleString('pt-BR')}` : ''}`}</span><small>{formatAcademicDate(item.assessmentDate)}</small></article>)}</div> : null}</> : <StateMessage title="Sem lançamentos no período" message="Ainda não existem notas ou avaliações disponíveis para o ano e período selecionados." />}</section> : null}
    {!loading && studentId && tab === 'attendance' ? <section className="family-panel"><h2>Frequência</h2>{attendance && attendance.classesCount > 0 ? <><p className="muted">Fonte: {attendance.source === 'DIARY' ? 'lançamentos do Diário de Classe' : 'resultado acadêmico consolidado'}.</p><div className="metric-grid"><MetricCard label="Aulas consideradas" value={attendance.classesCount.toLocaleString('pt-BR')} detail="Aulas registradas no período" /><MetricCard label="Faltas" value={attendance.absences.toLocaleString('pt-BR')} detail="Faltas e faltas justificadas" /><MetricCard label="Frequência" value={attendance.attendancePercent == null ? 'Sem base' : `${attendance.attendancePercent.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}%`} detail="(aulas - faltas) ÷ aulas × 100" /></div><div className="family-result-grid">{attendance.components.map((item) => <article className="family-result-card" key={item.componentName}><strong>{item.componentName}</strong><span>{item.attendancePercent == null ? 'Sem base de frequência' : `${item.attendancePercent.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}% de frequência`}</span><span>{item.absences} falta(s) em {item.classesCount} aula(s)</span></article>)}</div></> : <StateMessage title="Sem frequência no período" message="Ainda não existem lançamentos de frequência no Diário nem resultado consolidado para o ano e período selecionados." />}</section> : null}
